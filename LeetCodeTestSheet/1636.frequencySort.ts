...
function frequencySort(nums: number[]): number[] {
  ...
  //  finding the frequency using map/object
  for (const num of nums) {
    let key: number = num;
    freqMap.set(key, (freqMap.get(key) || 0) + 1);
  }

  // sort by the frequency / value
  const sortedValues: number[] = nums.sort((a: number, b: number): number => {
    const freqMapA: number | undefined = freqMap.get(a);
    const freqMapB: number | undefined = freqMap.get(b);

    if (freqMapA === freqMapB) return b - a;
    else return freqMapA - freqMapB;
  });
